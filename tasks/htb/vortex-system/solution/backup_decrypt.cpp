...
void generate_key(uint64_t timestamp) {
...
void decrypt_backup(std::string filename) {
    ...
    memcpy(iv, data + file_size - 16, 16);

    for (auto i = 0; i < 16; ++i) {
        std::cout << std::hex << (int)data[i] << " ";
    }
    std::cout << std::endl;
    const unsigned long decrypted_size = file_size - 16;
    std::vector<unsigned char> decrypted(decrypted_size);
    unsigned long padded_size = 0;

    plusaes::decrypt_cbc((unsigned char*)data, file_size - 16, &ekey[0], KEY_SIZE, &iv, &decrypted[0], decrypted.size(), &padded_size);

    std::ofstream out_file(filename + ".dec");
    out_file.write((const char*)decrypted.data(), decrypted.size());
    out_file.close();
}


int main(int argc, char* argv[]) {
    uint64_t timestamp = atoll(argv[2]);
